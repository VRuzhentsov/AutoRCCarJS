...
class CollectTrainingData {
  constructor(host, port, serial_port, input_size){
    ...
    this.connection = this.server_socket.accept()[0].makefile('rb')

    // connect to a serial port
    this.ser = serial.Serial(serial_port, 115200, 1)
    this.send_inst = true
  ...
  collect(){
    ...
    console.log("Press 'q' or 'x' to finish...")
    const start = cv2.getTickCount()

    const X = np.zeros((0, this.input_size))
    const y = np.zeros((0, 4))
